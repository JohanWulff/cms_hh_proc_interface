...
std::map<std::string, float> FeatComp::process(const TLorentzVector& b_1,
    ...
    if (FeatComp::_feat_check("dphi_httmet_met")) feats["dphi_httmet_mpt"] = FeatComp::delta_phi(h_tt_met, met);
    if (FeatComp::_feat_check("dphi_httvis_met")) feats["dphi_httvis_mpt"] = FeatComp::delta_phi(h_tt_vis, met);
    if (FeatComp::_feat_check("dphi_hbb_met"))    feats["dphi_hbb_met"]    = FeatComp::delta_phi(h_bb, met);
    if (FeatComp::_feat_check("dphi_hbb_sv"))     feats["dphi_hbb_sv"]     = FeatComp::delta_phi(h_bb, sv);
    ...
    if (FeatComp::_feat_check("deta_hbb_sv"))     feats["deta_hbb_sv"]     = FeatComp::delta_eta(h_bb, sv);
    ...
    if (FeatComp::_feat_check("dR_hbb_sv"))     feats["dR_hbb_sv"]     = h_bb.DeltaR(sv);
    ...
    if (FeatComp::_feat_check("dR_l1_l2_x_sv_pT"))       feats["dR_l1_l2_x_sv_pT"]       = l_1.DeltaR(l_2)*sv.Pt();
    ...
    if (FeatComp::_feat_check("dR_l1_l2_boosted_sv"))      feats["dR_l1_l2_boosted_sv"]      = FeatComp::delta_eta(l_1, l_2, sv);
    ...
    if (FeatComp::_feat_check("diH_mass_sv"))  feats["diH_mass_sv"]  = (h_bb+sv).M();
    ...
    if (FeatComp::_feat_check("phi2"))     feats["phi2"]     = FeatComp::calc_phi_1(b_1, b_2, sv, hh);
    if (FeatComp::_feat_check("costheta_star_met")) feats["costheta_star_met"] = FeatComp::calc_cos_delta_star(h_tt_met, h_bb+h_tt_met);
    if (FeatComp::_feat_check("costheta_star"))     feats["costheta_star"]     = FeatComp::calc_cos_delta_star(sv, hh);
    if (FeatComp::_feat_check("costheta_l1_httmet"))      feats["costheta_l1_httmet"]      = FeatComp::calc_cos_delta(l_1, h_tt_mpt);
    if (FeatComp::_feat_check("costheta_l1_htt"))         feats["costheta_l1_httmet"]      = FeatComp::calc_cos_delta(l_1, sv);
    if (FeatComp::_feat_check("costheta_l2_httmet"))      feats["costheta_l2_httmet"]      = FeatComp::calc_cos_delta(l_2, h_tt_mpt);
    if (FeatComp::_feat_check("costheta_l2_htt"))         feats["costheta_l2_httmet"]      = FeatComp::calc_cos_delta(l_2, sv);
    if (FeatComp::_feat_check("costheta_met_httmet"))     feats["costheta_met_httmet"]     = FeatComp::calc_cos_delta(met, h_tt_mpt);
    if (FeatComp::_feat_check("costheta_met_htt"))        feats["costheta_met_htt"]        = FeatComp::calc_cos_delta(met, sv);
    ...
    if (FeatComp::_feat_check("costheta_htt_hh_vis"))     feats["costheta_htt_hh_vis"]     = FeatComp::calc_cos_delta(sv, h_tt_vis+h_bb);
...
bool FeatComp::_feat_check(std::string feat) {
    if (_all) return true;
    return _requested.find(feat) != _requested.end()
}
